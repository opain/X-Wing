...
cat("* Version 1.0.0 \n")
cat("* Step1: Local genetic correlation detector (LOGODetect) \n")
...
cat(paste0("--ref_dir ", opt$ref_dir, " \\ \n"))
cat(paste0("--pop ", opt$pop, " \\ \n"))
cat(paste0("--block_partition", opt$block_partition, " \\ \n"))
cat(paste0("--gc_snp ", opt$gc_snp, " \\ \n"))
cat(paste0("--out_dir ", opt$out_dir, " \\ \n"))
if (!is.na(opt$target_pop)) {
  cat(paste0("--target_pop ", opt$target_pop, " \\ \n"))
}
if (!is.na(opt$n_topregion)) {
  cat(paste0("--n_topregion ", opt$n_topregion, " \\ \n"))
}
...
############################### use XPASS to caluculate heritability and cross-population genetic correlation
...
sfExport('scan', 'svd.try', 'simulate_zscore_helper', 'cal_qmax')
Result = sfLapply(1:nrow(block), apply.fun)
sfStop()
...
############################### use XPASS to calculate cross-population genetic correlation in aggregated regions
xpass_theta = list()
for(j in 1:length(theta)){
  if(nrow(Result[[j]])>0){
